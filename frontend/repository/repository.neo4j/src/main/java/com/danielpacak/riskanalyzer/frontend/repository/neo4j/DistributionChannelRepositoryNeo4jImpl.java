...
public class DistributionChannelRepositoryNeo4jImpl implements DistributionChannelRepository {

	GraphDatabaseService databaseService;

	public DistributionChannelRepositoryNeo4jImpl(GraphDatabaseService databaseService) {
	...
	public DistributionChannel save(DistributionChannel distributionChannel, String sourceId, String targetId) {
		...
		try {
			Node sourceNode = databaseService.getNodeById(Long.valueOf(sourceId));
			Node targetNode = databaseService.getNodeById(Long.valueOf(targetId));

			Relationship relationship = null;

			if (isBlank(distributionChannel.getId())) {
				relationship = sourceNode.createRelationshipTo(targetNode, FacilityRelationship.distributesTo);
				distributionChannel.setId(String.valueOf(relationship.getId()));
			} else {
				relationship = databaseService.getRelationshipById(Long.valueOf(distributionChannel.getId()));
			}

			relationship.setProperty("purchasingVolume", distributionChannel.getPurchasingVolume());
			transaction.success();
	...
	public void delete(String distributionChannelId) {
		Transaction transaction = databaseService.beginTx();
		try {
			Relationship relationship = databaseService.getRelationshipById(Long.valueOf(distributionChannelId));
			relationship.delete();
			transaction.success();
		} finally {
			transaction.finish();
	...
	public DistributionChannel findOne(String distributionChannelId) {
		...
		channel.setPurchasingVolume((Double) relationship.getProperty("purchasingVolume"));
		...
		FacilityRepository facilityRepository = new FacilityRepositoryNeo4jImpl(databaseService);
	...
	public List<DistributionChannel> findAll() {
		Iterator<Relationship> iterator = GlobalGraphOperations.at(databaseService).getAllRelationships().iterator();
